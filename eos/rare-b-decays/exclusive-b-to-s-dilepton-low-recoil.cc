...
namespace eos
{
    ...
    //lepton universality
    double
    BToKstarDilepton<LowRecoil>::differential_ratio_muons_electrons(const double & s) const
    {
        double br_electrons;
        {
            Save<Parameter, double> save_m_l(_imp->m_l, _imp->parameters["mass::e"]());
            br_electrons = BToKstarDilepton<LowRecoil>::differential_branching_ratio(s);
        }

        double br_muons;
        {
            Save<Parameter, double> save_m_l(_imp->m_l, _imp->parameters["mass::mu"]());
            br_muons = BToKstarDilepton<LowRecoil>::differential_branching_ratio(s);
        }
    ...
    double
    BToKstarDilepton<LowRecoil>::integrated_ratio_muons_electrons(const double & s_min, const double & s_max) const
    {
        std::function<double (const double &)> integrand = std::bind(std::mem_fn(&BToKstarDilepton<LowRecoil>::differential_branching_ratio),
    ...
    double 
    BToKstarDilepton<LowRecoil>::mag_amp_perp(const double & s) const
    {
        Amplitudes amp = _imp->amplitudes(s);
        return std::abs( amp.a_perp_left ) + std::abs( amp.a_perp_right ) ;
    }
    double 
    BToKstarDilepton<LowRecoil>::mag_amp_para(const double & s) const
    {
        Amplitudes amp = _imp->amplitudes(s);
        return std::abs( amp.a_par_left ) + std::abs( amp.a_par_right ) ;
    }
    double 
    BToKstarDilepton<LowRecoil>::mag_amp_long(const double & s) const
    {
        Amplitudes amp = _imp->amplitudes(s);
